...
def dtMap(detpair,grid=15,pngFile='',plottype='imshow',fignum=None,units='ms',colormap='jet'):
    ...
    if plottype=="contour":
        ...
        nRA=int(360/gridsize[0])
        nDec=int(180/gridsize[1])
        gridRA=np.arange(gridsize[0]/2,360,gridsize[0])
        gridDec=np.arange(90-gridsize[1]/2,-90,-gridsize[1])
    ...
    if plottype=="contour":
        ...
        dRAgrid=nRA/24
        dDecgrid=nDec/12
        plt.imshow(dtarr,aspect='equal',cmap=cmap,vmin=cmin,vmax=cmax)
        ax=plt.gca()
        ax.set_aspect('equal')
        # add ticks and labels
...
def plotMaps(detpairs,grid=15,plotDir='',dataDir='',plottype='imshow',
    ...
    p=0
    for dd in detpairs:
        print('plotting dt map for {}'.format(detpairs[dd].name))
        p=p+1
        ...
        dtMap(detpairs[dd],pngFile=pngFile,fignum=p,plottype=plottype,units=units,colormap=colormap)
    plt.show()