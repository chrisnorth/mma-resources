function Step(data,opt){
  ...
  el = {
	...
	this.updateGraph = function(){
		...
		var output = "<div>T<sub>90</sub> = {{ t90 }} seconds, T<sub>100</sub> = {{ t100 }} seconds, baseline = {{ baseline }} counts/s, F<sub>90</sub> = {{ F90 }} counts, F<sub>100</sub> = {{ F100 }} counts, ratio = {{ ratio }} {{ indicator }}</div>";
		var msg = "";
		if(ratio > 0.95) msg = "Too high";
		if(ratio < 0.85) msg = "Too low";
	...
	this.setEvent = function(e){
		...
		if(e){
			if(data.events[e]){
				...
				function label(d){
					var txt = "{{ title }}<br />Time: {{ x }} s<br />Value: {{ y }} counts/s";
				...
				var axes = {
					'x':{
						'min': Infinity,
						'max': -Infinity,
						'title': { 'label': 'Time (seconds)', 'attr': { 'fill': 'black' } }
					},
					'y':{
						'min': Infinity,
						'max': -Infinity,
						'title': { 'label':'Count rate (counts/s)', 'attr': { 'fill': 'black' } },
				...
				this.graph.setSeries("F100",series,{
					'title': {
						'label': 'F100 - total counts above the baseline.'
					},
					'line': {
						'stroke': ''
					},
