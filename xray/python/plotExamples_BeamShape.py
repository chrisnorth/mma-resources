...
for m in models:
    ...
    ax.annotate(r'{} {:.1f}$^\circ$'.format(str(models[m]['pars']['jetType']).replace('0','Gaussian').replace('-1','Top Hat'),np.rad2deg(models[m]['pars']['thetaObs'])),(0.01,0.95),xycoords='axes fraction',va='top')
...
print("Saving example_light_curves_beamshape.png")
fig.savefig(os.path.join(plotDir,"example_light_curves_beamshape.png"))
# plt.close(fig)
plt.show()